...
//在线协议表
let arduinoProtocol = {
    set_figure: (index, port, level) => [
        ['byte', index], '021e', ['byte', port],
        ['byte', level]
    ],
...
const disableBlocks = {
    debug: ["arduino_for_rj25.BLOCK_1596451673116", "arduino_for_rj25.BLOCK_1596451712930", "arduino_for_rj25.BLOCK_1596451771370"],
    upload: []
};
        ...
        this.funcs = {
            'BLOCK_1596436351300': {
                onRun: async (args, app, device, block) => {
                    ...
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '011e', ['byte', Number(args.number_1)]
                    ]);
            ...
            'BLOCK_1596436396122': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '011f', ['byte', Number(args.number_1)]
                    ]);
                    let value = await device.asyncReadProtocol('ff55', [`${byte2Hex(index)}02`, 'float']);
                    resetIndex(index)
                    if (value == null) {
            ...
            'block1596436433921CodesCode': (args) => {
                if ((args.number_2 == "") || (args.number_2 == null) || (args.number_2 == "None") || (args.number_2 < 0)) {
                    args.number_2 = 0;
                }
                return `pulseIn(${args.number_1},HIGH,${args.number_2})`
            },
            'BLOCK_1596436433921': {
                onRun: async (args, app, device, block) => {
                    if ((args.number_2 == "" || args.number_2 == null) || (args.number_2 == "None") || (args.number_2 < 0)) {
                    ...
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '0125', ['byte', Number(args.number_1)],
                        ['short', Number(args.number_2)]
                    ]);
            ...
            'BLOCK_1596436491466': {
                onRun: async (args, app, device, block) => {
                    let number_1 = Number(args.number_1);
                    let fieldMenu_2 = Number(args.fieldMenu_2);
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', arduinoProtocol.set_figure(index, number_1, fieldMenu_2));
                    await device.asyncReadProtocol('ff55', []);
                    resetIndex(index);
                }
            },
            'block1596437190274CodesCode': (args) => {
                if ((args.number_2 == "") || (args.number_2 == null) || (args.number_2 == "None") || (args.number_2 < 0)) {
                    args.number_2 = 0;
                } else if (args.number_2 > 255) {
                    args.number_2 = 255;
                }
                return `
analogWrite(${args.number_1},${args.number_2});`
            },
            'BLOCK_1596437190274': {
                onRun: async (args, app, device, block) => {
                    if ((args.number_2 == "" || args.number_2 == null) || (args.number_2 == "None") || (args.number_2 < 0)) {
                    ...
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '0220', ['byte', Number(args.number_1)],
                        ['byte', Number(args.number_2)]
                    ]);
                    await device.asyncReadProtocol('ff55', []);
                    resetIndex(index);
                }
            ...
            'BLOCK_1596442885175': {
                onRun: async (args, app, device, block) => {
                    let number_1 = Number(args.number_1);
                    let fieldMenu_2 = Number(args.fieldMenu_2);
                    let inputMenu_3 = Number(args.inputMenu_3);
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '0222', ['byte', number_1],
                        ['short', fieldMenu_2],
                        ['short', inputMenu_3]
                    ]);
                    await device.asyncReadProtocol('ff55', []);
            ...
            'block1596443222386CodesLib': ['Servo servo_/*{number_1}*/;'],
            'block1596443222386CodesCode': (args) => {
                if ((args.number_2 == "") || (args.number_2 == null) || (args.number_2 == "None") || (args.number_2 < 0)) {
                    args.number_2 = 0;
                } else if (args.number_2 > 180) {
                    args.number_2 = 180;
                }
                return `
servo_${args.number_1}.write(${args.number_2});`
            },
            'BLOCK_1596443222386': {
                onRun: async (args, app, device, block) => {
                    if ((args.number_2 == "" || args.number_2 == null) || (args.number_2 == "None") || (args.number_2 < 0)) {
                    ...
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '0221', ['byte', Number(args.number_1)],
                        ['byte', Number(args.number_2)]
                    ]);
                    await device.asyncReadProtocol('ff55', []);
                    resetIndex(index);
            ...
            'BLOCK_1596451799277': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
                        ['byte', index], '0124', ['byte', Number(args.number_1)],
                        ['byte', Number(args.number_2)]
                    ]);
                    let value = await device.asyncReadProtocol('ff55', [`${byte2Hex(index)}02`, 'float']);
                    resetIndex(index)
    ...
    getInfo() {
        return {
            ...
            "disabledOffline": [],
            "disabledOnline": [
                "arduino_for_rj25.BLOCK_1596451673116",
                "arduino_for_rj25.BLOCK_1596451712930",
                "arduino_for_rj25.BLOCK_1596451771370"
            ]
        };
    }
...
export default ExtArduinoForRj25;