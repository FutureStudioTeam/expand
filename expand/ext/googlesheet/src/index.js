...
const MAX_COL = 52; //-az
let url = {
    connectURL: (id) => `https://msservice.mblock.cc/api/v1/spreadsheets/${id}/${MAX_ROW}/${MAX_COL}/append`, //连接sheet并初始化表格行数列数
    valueURL: (id, col, row) => `https://msservice.mblock.cc/api/v1/spreadsheets/${id}/${row}/${col}/value`,
}
...
function connectGoogle(id) {
    return new Promise(function(resolve, reject) {
        requestHttp('POST', {
            url: url.connectURL(id)
        }, function(result) {
            resolve(result)
        });
    })
...
function getValue(id, col, row) {
    return new Promise(resolve => {
        if (col === '' || col == null || isNaN(col)) {
            return "";
        }
        requestHttp('GET', {
            url: url.valueURL(id, col, row)
        }, (result) => {
            resolve(result)
        });
    })
...
function postValue(id, col, row, value) {
    return new Promise(resolve => {
        requestHttp('POST', {
            url: url.valueURL(id, col, row),
            data: {
...
const msgWidget = async (msg, type, duration) => {
    let requestTimer = undefined;

    const service = _appContext.getService('system.ui');
    return await service.message(msg, type, duration);
};
        ...
        this.funcs = {
            'block1552896553516': {
                onRun: async (args, app, device, block) => {
                    if (!args.url) return msgWidget('googlesheet.connect_fail', 'warn', 2);
                    try {
                        this.spreadsheetId = new RegExp("https://docs.google.com/spreadsheets/d/([a-zA-Z0-9-_]+)").exec(args.url)[1];
                        if (!this.spreadsheetId) return msgWidget('googlesheet.connect_fail', 'warn', 2);
                        var res = await connectGoogle(this.spreadsheetId, args.col, args.row, args.value);
                        await delay(200);
                        console.log(res);
                        const data = JSON.parse(res);
                        data.code === 0 ?
                            msgWidget('googlesheet.connect_success', 'success', 2) :
                            msgWidget('googlesheet.connect_fail', 'warn', 2);
                    } catch (err) {
                        console.log(err);
                        msgWidget('googlesheet.connect_fail', 'warn', 2);
                    }
            ...
            'block1552896647935': {
                onRun: async (args, app, device, block) => {
                    if (!isPositiveInt(args.col) || !isPositiveInt(args.row)) return;
                    if (args.value === '') return
                    if (args.col > MAX_COL || args.row > MAX_ROW) return;
                    let value = isNumeric(args.value) ? +args.value : args.value.toString();
                    var data = await postValue(this.spreadsheetId, args.col, args.row, value);
                    await delay(200);
                    console.log(data)
                }
            },
            'block1552896647936': {
                onRun: async (args, app, device, block) => {
                    ...
                    if (args.col > MAX_COL || args.row > MAX_ROW) return '';
                    try {
                        const res = await getValue(this.spreadsheetId, args.col, args.row);
                        console.log(res);
                        const data = JSON.parse(res);
                        await delay(200);
