...
const halocodeMbuildAngleSensorHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildAngleSensorTable = [
        new onlineDataStructure("BLOCK_1559703114098", "mbuild.angle_sensor.get_angle", [1], {
            default: "int"
        }),
        new onlineDataStructure("BLOCK_1559703352577", "mbuild.angle_sensor.reset_angle", [1]),
        new onlineDataStructure("BLOCK_1559703305709", "mbuild.angle_sensor.get_angle_speed", [1], {
            default: "int"
        }),
        new onlineDataStructure("BLOCK_1559703446126_clockwise", "mbuild.angle_sensor.is_rotating_clockwise", [1], {
            default: "bool"
        }),
        new onlineDataStructure("BLOCK_1559703446126_anticlockwise", "mbuild.angle_sensor.is_rotating_anticlockwise", [1], {
            default: "bool"
        }),
    ]
    onlineManager.updateOnlineTable("add", mbuildAngleSensorTable);
}
...
async function set_angle_sensor_report_interval(value) {
    if (onlineCommunication) {
        await onlineCommunication.write(onlineProtocolPackage.__createFrame(0x04, 0, "mbuild.angle_sensor.set_report_mode(2, " + value + ", 1)", null, true));
    }
}

let angleSensorReportSetHandle = setInterval(async () => {
    await set_angle_sensor_report_interval(100);
}, 3000)
...
class ExtHalocodeMbuildAngleSensor {
        ...
        this.funcs = {
            ...
            'BLOCK_1559703352577': {
                onRun: async (args, app, device, block) => {
                    let opcode = "BLOCK_1559703352577";
                    let para = [Number(args.index)];

                    onlineManager.parasUpdate(opcode, para);
                    await onlineManager.request(opcode);

                    // the value of angle should be reset
                    let key = onlineManager.__getKeyViaTag("BLOCK_1559703114098");
                    if (onlineManager.blockTable[key].value[args.index] !== undefined) {
            ...
            'BLOCK_1559703305709': {
                onRun: async (args, app, device, block) => {
                    ...
                    let para = [Number(args.index)];
                    let opcode = "BLOCK_1559703305709";
            ...
            'BLOCK_1559703446126': {
                onRun: async (args, app, device, block) => {
                    ...
                    let para = [Number(args.index)];
                    let opcode = "BLOCK_1559703446126" + '_' + args.dir;
    ...
    getInfo() {
        return {
            "id": "halocode_mbuild_angle_sensor",
            "targets": [
                "haloboard",
                "cyberpi"
            ],
...
export default ExtHalocodeMbuildAngleSensor;