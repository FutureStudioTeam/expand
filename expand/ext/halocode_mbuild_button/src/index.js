...
const halocodeMbuildButtonHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildButtonTable = [
        new onlineDataStructure("BLOCK_1559700086192", "mbuild.button.is_pressed", [1], {
            default: "bool"
        }),
        new onlineDataStructure("BLOCK_1559700149950", "mbuild.button.get_count", [1], {
            default: "int"
        }),
        new onlineDataStructure("BLOCK_1559700240893", "mbuild.button.reset_count", [1]),
    ]

    onlineManager.updateOnlineTable("add", mbuildButtonTable);
}
        ...
        this.funcs = {
            'BLOCK_1559700086192': {
                onRun: (args, app, device, block) => {
                    ...
                    const opcode = "BLOCK_1559700086192";
            ...
            'BLOCK_1559700149950': {
                onRun: (args, app, device, block) => {
                    ...
                    const opcode = "BLOCK_1559700149950";
                    let para = [Number(args.index)];
                    onlineManager.parasUpdate(opcode, para, args.index);
                    let value = onlineManager.getValue(opcode, args.index);
            ...
            'BLOCK_1559700240893': {
                onRun: async (args, app, device, block) => {
                    ...
                    const opcode = "BLOCK_1559700240893";
                    let para = [Number(args.index)];
                    onlineManager.parasUpdate(opcode, para);
                    await onlineManager.request(opcode);

                    // the value of button count should be reset
                    let key = onlineManager.__getKeyViaTag("BLOCK_1559700149950");
            ...
            "targets": [{
                    ...
                    "options": {
                        "upload": {
                            "middlewares": [{
                                "name": "temp",
                                "func": halocodeMbuildButtonHaloboardUploadMiddlewareTemp
                            }]
                        }
                    }
                },
...
export default ExtHalocodeMbuildButton;