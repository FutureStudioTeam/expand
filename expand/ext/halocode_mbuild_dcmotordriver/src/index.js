...
const halocodeMbuildDcmotordriverHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildDcMotorTable = [
        // {args.power}, {args.index}
        new onlineDataStructure("BLOCK_1550542693995", "mbuild.dc_motor_driver.set_power", [1]),
        // {power}, {index}
        new onlineDataStructure("BLOCK_1550542713045", "mbuild.dc_motor_driver.change_power", [1]),
        // 0, {args.index}
        new onlineDataStructure("BLOCK_1550543155224", "mbuild.dc_motor_driver.stop_all", []),
        new onlineDataStructure("BLOCK_1550543162879", "mbuild.dc_motor_driver.get_power", [1], {
            default: "int"
        }),
    ]
    onlineManager.updateOnlineTable("add", mbuildDcMotorTable);
}
...
class ExtHalocodeMbuildDcmotordriver {

    constructor() {
        this.funcs = {
            'block1550542693995CodesCode': (args) => {
                if (args.time == "None" || (args.time < 0)) {
                    args.time = 0;
                }
                return `mbuild.dc_motor_driver.set_power(${args.power}, ${args.index})
time.sleep(${args.time})
mbuild.dc_motor_driver.set_power(0, ${args.index})`
            },
            'BLOCK_1550542693995': {
            ...
            'BLOCK_1550542713045': {
                onRun: async (args, app, device, block) => {
                    let opcode = "BLOCK_1550542713045";
                    let para = [Number(args.power), Number(args.index)];
            ...
            'BLOCK_1550543155224': {
                onRun: async (args, app, device, block) => {
                    let opcode = "BLOCK_1550543155224";
                    await onlineManager.request(opcode);
                }
            },
            'BLOCK_1550543162879': {
                onRun: async (args, app, device, block) => {
                    ...
                    let opcode = "BLOCK_1550543162879";
    ...
    getInfo() {
        return {
            "id": "halocode_mbuild_dcmotordriver",
            "targets": [{
                    "name": "haloboard",
                    "options": {
                        "upload": {
                            "middlewares": [{
                                "name": "temp",
                                "func": halocodeMbuildDcmotordriverHaloboardUploadMiddlewareTemp
                            }]
                        }
                    }
                },
...
export default ExtHalocodeMbuildDcmotordriver;