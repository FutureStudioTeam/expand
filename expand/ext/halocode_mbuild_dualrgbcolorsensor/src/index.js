...
const halocodeMbuildDualrgbcolorsensorHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildDualRgbSensorTable = [
        // '{prob}', '{color}', {index}
        new onlineDataStructure("DUAL_RGB_IS_COLOR", "mbuild.dual_rgb_sensor.get_color", ["ALL", 1], {
            default: "array"
        }),
        // '{prob}', {index}
        new onlineDataStructure("DUAL_RGB_GET_INTENSITY_RGB1", "mbuild.dual_rgb_sensor.get_intensity", ["RGB1", 1], {
            default: "int"
        }),
        new onlineDataStructure("DUAL_RGB_GET_INTENSITY_RGB2", "mbuild.dual_rgb_sensor.get_intensity", ["RGB2", 1], {
            default: "int"
        }),
    ]
    onlineManager.updateOnlineTable("add", mbuildDualRgbSensorTable);
...
async function set_rgb_sensor_report_interval(value) {
    if (onlineCommunication) {
        value = 80;
        await onlineCommunication.write(onlineProtocolPackage.__createFrame(0x04, 0, "mbuild.dual_rgb_sensor.__REPORT_TIME = " + value, null, true));

    }
}
let rgbReportSetHabdle = setInterval(async () => {
    await set_rgb_sensor_report_interval(100);
}, 3000)
...
class ExtHalocodeMbuildDualrgbcolorsensor {
        ...
        this.funcs = {
            'DUAL_RGB_IS_COLOR': {
                onRun: (args, app, device, block) => {
                    ...
                    let para = ["ALL", Number(args.index)];
                    let opcode = "DUAL_RGB_IS_COLOR";

                    onlineManager.parasUpdate(opcode, para, args.index);
                    let value = onlineManager.getValue(opcode, args.index, "array", (originValue) => {
                        ...
                        return !!(originValue[channel] === args.color);
                    });
                    ...
                    if (!(value instanceof Promise)) {
                        ...
                        return !!(value[channel] === args.color);
                    }
            ...
            'DUAL_RGB_GET_INTENSITY': {
                onRun: (args, app, device, block) => {
                    if (!device.isConnected() || device.isUploadMode()) return 0;

                    let para = [args.prob, Number(args.index)];
                    let opcode = "DUAL_RGB_GET_INTENSITY" + '_' + args.prob;
            ...
            'DUAL_RGB_GET_INFLECTION_INTENSITY': {
                onRun: (args, app, device, block) => {
                    // TODO
                },
                onMonitor: (app, device, block) => {
            ...
            'DUAL_RGB_SET_SUPPLEMENT_LIGHT_WITH_FACE': {
                onRun: (args, app, device, block) => {
                    // TODO
                },
                onMonitor: (app, device, block) => {
            ...
            'DUAL_RGB_SET_SUPPLEMENT_LIGHT_WITH_RGB': {
                onRun: (args, app, device, block) => {
                    // TODO
                }
            }
    ...
    getInfo() {
        return {
            "id": "halocode_mbuild_dualrgbcolorsensor",
            "targets": [
                "haloboard",
                "cyberpi"
            ],
...
export default ExtHalocodeMbuildDualrgbcolorsensor;