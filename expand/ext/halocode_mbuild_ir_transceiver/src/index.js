...
const halocodeMbuildIrTransceiverHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildIrTransceiverTable = [
        // {message}, {index}
        new onlineDataStructure("BLOCK_1559548002545", "mbuild.ir_transceiver.send", ["hello", 1]),
        new onlineDataStructure("BLOCK_1559640370045", "mbuild.ir_transceiver.receive", [1], {
            default: "string"
        }),
        //{record_id}, {index}
        new onlineDataStructure("BLOCK_1559641582335", "mbuild.ir_transceiver.__learn_online", [1, 1]),
        new onlineDataStructure("BLOCK_1559641607063", "mbuild.ir_transceiver.send_learned_result", [1, 1]),

        new onlineDataStructure("BLOCK_1559641693311", "mbuild.ir_transceiver.receive_remote_code", [1], {
            default: "array"
        }),
    ]

    onlineManager.updateOnlineTable("add", mbuildIrTransceiverTable);
}
...
class ExtHalocodeMbuildIrTransceiver {

    constructor() {
        this.funcs = {
            'BLOCK_1559548002545': {
                onRun: async (args, app, device, block) => {
                    let opcode = "BLOCK_1559548002545";
                    let para = [args.message, Number(args.index)];
            ...
            'BLOCK_1559640370045': {
                onRun: (args, app, device, block) => {
                    ...
                    let opcode = "BLOCK_1559640370045";
            ...
            'BLOCK_1559641582335': {
                onRun: async (args, app, device, block) => {
                    let opcode = "BLOCK_1559641582335";
                    let para = [Number(args.record_id), Number(args.index)];

                    // let waitTime = 500;
                    // onlineManager.parasUpdate(opcode, para);
                    // await onlineManager.request(opcode, true, waitTime);

                    // a secial process for a bug in device

                    let recordId = Number(args.record_id) - 1;

                    await onlineCommunication.write(onlineProtocolPackage.__createFrame(0x04, 0,
                        "mbuild.ir_transceiver.neurons_request(\"m_ir_sensor\", \"record\", (3000," + recordId + ")," + args.index + ")", null, true));

                    // learn time 3000s defalut 
                    await commonFunction.delay(3000);

                }
            },
            'BLOCK_1559641607063': {
                onRun: async (args, app, device, block) => {
                    let opcode = "BLOCK_1559641607063";
                    let para = [Number(args.record_id), Number(args.index)];
            ...
            'BLOCK_1559641693311': {
                onRun: (args, app, device, block) => {
                    ...
                    let para = [Number(args.index)];
                    let opcode = "BLOCK_1559641693311";

                    onlineManager.parasUpdate(opcode, para, args.index);
                    let value = onlineManager.getValue(opcode, args.index, undefined, (originValue) => {
                        return !!(originValue[1] === eval(args.cmd)[1]);
                    });

                    if (!(value instanceof Promise)) {
                        return !!(value[1] === eval(args.cmd)[1]);
                    }
    ...
    getInfo() {
        return {
            "id": "halocode_mbuild_ir_transceiver",
            "targets": [
                "haloboard",
                "cyberpi"
            ],
...
export default ExtHalocodeMbuildIrTransceiver;