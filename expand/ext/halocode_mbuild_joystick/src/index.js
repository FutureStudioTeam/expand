...
const halocodeMbuildJoystickHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildJoystickTable = [
        // '{dir}', {index}  -- up down left right
        new onlineDataStructure("BLOCK_1559707965944", "mbuild.joystick.is_active", ['up', 1], {
            default: "array"
        }),
        // '{axis}', {index}  -- up down left right
        new onlineDataStructure("BLOCK_1559708436297", "mbuild.joystick.get_value", ['x', 1], {
            default: "array"
        }),
    ]


    onlineManager.updateOnlineTable("add", mbuildJoystickTable);
}
...
class ExtHalocodeMbuildJoystick {
            ...
            'BLOCK_1559707965944': {
                onRun: (args, app, device, block) => {
                    ...
                    if (!(value instanceof Promise)) {
                        if (args.dir === "up") {
                            return !!(value[1] > ACTIVE_THRESHOLD)
                        } else if (args.dir === "down") {
                            return !!(value[1] < -ACTIVE_THRESHOLD)
                        } else if (args.dir === "left") {
                            return !!(value[0] < -ACTIVE_THRESHOLD)
                        } else if (args.dir == "right") {
                            return !!(value[0] > ACTIVE_THRESHOLD)
                        }
                    }
...
export default ExtHalocodeMbuildJoystick;