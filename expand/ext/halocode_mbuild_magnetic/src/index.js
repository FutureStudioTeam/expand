...
const halocodeMbuildMagneticHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildMagneticTable = [
        new onlineDataStructure("BLOCK_1565579151187", "mbuild.magnetic_sensor.is_active", [1], {
            default: "bool"
        }),
        new onlineDataStructure("BLOCK_1565579556728", "mbuild.magnetic_sensor.get_count", [1], {
            default: "int"
        }),
        new onlineDataStructure("BLOCK_1565579668227", "mbuild.magnetic_sensor.reset_count", [1]),
    ]

    onlineManager.updateOnlineTable("add", mbuildMagneticTable);
}
...
class ExtHalocodeMbuildMagnetic {
        ...
        this.funcs = {
            'BLOCK_1565579151187': {
                onRun: (args, app, device, block) => {
                    ...
                    const opcode = "BLOCK_1565579151187";
            ...
            'BLOCK_1565579556728': {
                onRun: (args, app, device, block) => {
                    ...
                    const opcode = "BLOCK_1565579556728";
                    let para = [Number(args.index)];
                    onlineManager.parasUpdate(opcode, para, args.index);
                    let value = onlineManager.getValue(opcode, args.index);
            ...
            'BLOCK_1565579668227': {
                onRun: async (args, app, device, block) => {
                    ...
                    const opcode = "BLOCK_1565579668227";
                    let para = [Number(args.index)];
                    onlineManager.parasUpdate(opcode, para);
                    await onlineManager.request(opcode);

                    // the value of magnetic sensor count should be reset
                    let key = onlineManager.__getKeyViaTag("BLOCK_1565579556728");
            ...
            "targets": [{
                    ...
                    "options": {
                        "upload": {
                            "middlewares": [{
                                "name": "temp",
                                "func": halocodeMbuildMagneticHaloboardUploadMiddlewareTemp
                            }]
                        }
                    }
                },
...
export default ExtHalocodeMbuildMagnetic;