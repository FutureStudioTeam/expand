...
const halocodeMbuildPirsensorHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildPirSensorTable = [
        new onlineDataStructure("event_pir", "mbuild.pir_sensor.is_activated", [1], {
            default: "bool"
        }),
        new onlineDataStructure("pir_count", "mbuild.pir_sensor.get_count", [1], {
            default: "int"
        }),
        new onlineDataStructure("pir_count_make_zero", "mbuild.pir_sensor.reset_count", [1]),
    ]

    onlineManager.updateOnlineTable("add", mbuildPirSensorTable);
}
...
class ExtHalocodeMbuildPirsensor {
        ...
        this.funcs = {
            ...
            'pir_count_make_zero': {
                onRun: async (args, app, device, block) => {
                    ...
                    let para = [Number(args.index)];
                    let opcode = "pir_count_make_zero";

                    onlineManager.parasUpdate(opcode, para);
                    let value = await onlineManager.request(opcode);

                    // the value of pir count should be reset
                    let key = onlineManager.__getKeyViaTag("pir_count");
...
export default ExtHalocodeMbuildPirsensor;