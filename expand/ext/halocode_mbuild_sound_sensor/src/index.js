...
const halocodeMbuildSoundSensorHaloboardUploadMiddlewareTemp =
    /**
...
function updateShareDatas() {
    ...
    let mbuildSoundSensorTable = [
        new onlineDataStructure("BLOCK_1559814177378", "mbuild.sound_sensor.get_loudness", [1], {
            default: "int"
        }),
    ]

    onlineManager.updateOnlineTable("add", mbuildSoundSensorTable)
}

function syncShareDatasFromDevice() {
    if (window.haloSharedObject) {
        updateShareDatas();
    } else {
        setTimeout(() => {
            updateShareDatas();
...
class ExtHalocodeMbuildSoundSensor {
        ...
        this.funcs = {
            'BLOCK_1559814177378': {
                onRun: (args, app, device, block) => {
                    ...
                    let opcode = "BLOCK_1559814177378";
    ...
    getInfo() {
        return {
            "id": "halocode_mbuild_sound_sensor",
            "targets": [{
                    "name": "haloboard",
                    "options": {
                        "upload": {
                            "middlewares": [{
                                "name": "temp",
                                "func": halocodeMbuildSoundSensorHaloboardUploadMiddlewareTemp
                            }]
                        }
                    }
                },
...
export default ExtHalocodeMbuildSoundSensor;