...
const halocodeUploadMessageHaloboardUploadMiddlewareTemp =
    /**
...
let uploadMessageID = 0x03;
let lowComputerSubscribeHandle = null;
...
function createMessageProtocol(message, value) {
    var frame = [];
    var scriptArray = [];
    var messageItem = {};
    if (!value) {
    ...
    var messageStr = JSON.stringify(messageItem);
    scriptArray = __strToArray(messageStr);

    frame = [uploadMessageID]; //[id, index, length]
    frame = frame.concat(scriptArray);
    frame = frame.concat(0x00);
    return frame;
}

async function broadcast(message, value) {
    if (engine) {
        /* the type of imput parametese is string */
        if (isNumber(value)) {
            value = Number(value);
        }
        await engine.asyncWriteProtocol('f3f4', createMessageProtocol(message, value));
        setTimeout(() => {
            appContext.sendMessage("robot2spriteBroadcast", {
                type: 'cmd',
                msg: 'threadFinish'
            })
        }, 5)
...
function updateContent(device, app) {
    engine = device;
    appContext = app;
}
...
async function subscribeLowerComputerMessage(reSubscribeFlag = false) {
    let subscribeDoFlag = false;

    if (reSubscribeFlag) {
        subscribeDoFlag = true;
        if (lowComputerSubscribeHandle) {
            lowComputerSubscribeHandle();
            lowComputerSubscribeHandle = null;
        }
    } else {
        if (lowComputerSubscribeHandle === null) {
            subscribeDoFlag = true;
        }
    }

    if (subscribeDoFlag === true) {
        lowComputerSubscribeHandle = await engine.subscribeReadProtocol('f3f4', [uploadMessageID, 'string0'], (values) => {
            if (values === null) {
                return null;
            }
            try {
                let messageStr = values[0];
                let messageJson = JSON.parse(messageStr);
                // console.log("message json", messageJson);
                broadcastMap[messageJson["message"]] = messageJson["value"];
                let resultData = {
...
function unSubscribeLowerComputerMessage() {
    lowComputerSubscribeHandle && lowComputerSubscribeHandle();
}

const disableBlocks = {
    debug: ["halocode_upload_message.BLOCK_1551748322055", "halocode_upload_message.BLOCK_1551748500023", "halocode_upload_message.BLOCK_1551748632732", "halocode_upload_message.BLOCK_1551748721826"],
    upload: []
};
...
class ExtHalocodeUploadMessage {
    ...
    getInfo() {
        return {
            ...
            "disabledOffline": [],
            "disabledOnline": [
                "halocode_upload_message.BLOCK_1551748322055",
                "halocode_upload_message.BLOCK_1551748500023",
                "halocode_upload_message.BLOCK_1551748632732",
                "halocode_upload_message.BLOCK_1551748721826"
            ]
        };
    }
...
export default ExtHalocodeUploadMessage;