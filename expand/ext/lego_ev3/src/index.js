...
class Scratch3Ev3Blocks {
    ...
    constructor(runtime) {
        ...
        peripheral = new EV3(this.runtime, Scratch3Ev3Blocks.EXTENSION_ID);
    ...
    getMotorPosition(args) {
        ...
        const motor = peripheral.motor(port);
        let position = 0;
        if (motor) {
...
let peripheral;
let ev3Instance = new Scratch3Ev3Blocks(fakeRuntime);
        ...
        this.funcs = {
            'BLOCK_1600227061719': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.motorTurnClockwise(args);

                }
            },
            'BLOCK_1600244245562': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.motorTurnCounterClockwise(args);

                }
            },
            'BLOCK_1600244267914': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.motorSetPower(args);

                }
            },
            'BLOCK_1600244969694': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.beep(args);
                }
            },
            'BLOCK_1600244352705': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.getMotorPosition(args);
                }
            },
            'BLOCK_1600244821767': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.buttonPressed(args);
                }
            },
            'BLOCK_1600244907594': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.getDistance();
                }
            },
            'BLOCK_1600244932319': {
                onRun: (args, app, device, block) => {
                    return ev3Instance.getBrightness();
                }
            },
            'BLOCK_1600244397480': {
                onRun: function(args, app, device, block) {
                    let cando = false;
                    const value = ev3Instance.whenButtonPressed(args);
                ...
                onAdd: (app, device, block) => {
                    // TODO
                    addListener('whenButtonPressed', () => {
                        app.workspace.runBlocks(block.opcode);
                    })
                },
                onRemove: (app, device, block) => {
                    removeListener('whenButtonPressed')
                }
            },
            'BLOCK_1600244527347': {
                onRun: function(args, app, device, block) {
                    let cando = false;
                    const value = ev3Instance.whenDistanceLessThan(args);
                    if (!this.oldValue && value) {
                        cando = true;
                    }
                    this.oldValue = value;
                ...
                onAdd: (app, device, block) => {
                    addListener('whenDistanceLessThan', () => {
                        app.workspace.runBlocks(block.opcode);
                    })
                },
                onRemove: (app, device, block) => {
                    removeListener('whenDistanceLessThan');
                }
            },
            'BLOCK_1600244758221': {
                onRun: function(args, app, device, block) {
                    let cando = false;
                    const value = ev3Instance.whenBrightnessLessThan(args);
                    if (!this.oldValue && value) {
                        cando = true;
                    }
                    this.oldValue = value;
                ...
                onAdd: (app, device, block) => {
                    addListener('whenBrightnessLessThan', () => {
                        app.workspace.runBlocks(block.opcode);
                    })
                },
                onRemove: (app, device, block) => {
                    removeListener('whenBrightnessLessThan')
                }
            },
    ...
    getInfo() {
        return {
            ...
            "targets": {
                "device": true,
                "name": "lego_ev3",
                "icon": window.MbApi.getExtResPath('lego_ev3/imgs/849e6ae3af6a4aef80b788be28bf1e93.png', 'lego_ev3'),
                "enableCode": true,
                "enableUpload": false,
                "enableOnline": [
                    "serialport"
                ...
                "options": {
                    "connect": {
                        "serialport": {
                            ...
                            "tips": {
                                "fail": [
                                    "lego_ev3.connect_fail_3d1d0949",
                                    "lego_ev3.connect_fail_b4c97367",
                                    "lego_ev3.connect_fail_9a92fc9a"
                                ]
                            },
                            "baudRate": "",
                            "vendorId": "",
