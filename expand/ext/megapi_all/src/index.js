...
const disableBlocks = {
    debug: ["megapi_all.megapi_detect_pm25_sensor", "megapi_all.enhanced_encoder_motor_power", "megapi_all.enhanced_encoder_motor_speed", "megapi_all.enhanced_encoder_motor_pos"],
    upload: []
};
...
class ExtMegapiAll {
    ...
    constructor() {
        this.funcs = {
            'megapiRunServoCodesLib': ['Servo servo_/*{PORT}*/_/*{SLOT}*/;',
                'MePort port_/*{PORT}*/(/*{PORT}*/);'
            ],
            'megapiRunServoCodesSetup': ['servo_/*{PORT}*/_/*{SLOT}*/.attach(port_/*{PORT}*/.pin/*{SLOT}*/());'],
            'megapiRunServoCodesCode': (args) => {
                if (args.DEGREE == "None") {
                    args.DEGREE = 0;
...
servo_${args.PORT}_${args.SLOT}.write(${args.DEGREE});`
            ...
            'megapiRunFanCodesLib': ['MePort dc130_/*{PORT}*/(/*{PORT}*/);'],
            'megapiRunFanCodesDeclare': [
    ...
    digitalWrite(dc130.pin2(), 0);
            ...
            'megapi_run_fan': {
                onRun: async (args, app, device, block) => {
                    ...
                    let direction = Number(args.FAN_ROTATE);
                    let index = await getIndex()
                    if (port == 6) {
                        if (direction == -1) {
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '3e', '00'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '3f', '01'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                        } else if (direction == 1) {
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '3e', '01'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '3f', '00'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                        } else {
                            await device.asyncWriteProtocol('ff55', [
                        ...
                        if (direction == -1) {
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '40', '00'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '41', '01'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                        } else if (direction == 1) {
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '40', '01'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '41', '00'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                        } else {
                            await device.asyncWriteProtocol('ff55', [
                        ...
                        if (direction == -1) {
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '43', '00'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '42', '01'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                        } else if (direction == 1) {
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '43', '01'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                            await device.asyncWriteProtocol('ff55', [
                                ['byte', index], '021e', '42', '00'
                            ]);
                            await device.asyncReadProtocol('ff55', []);
                        } else {
                            await device.asyncWriteProtocol('ff55', [
            ...
            'megapiShowExternalLedTimeCodesLib': ['MeRGBLed rgbled_/*{PORT}*/(/*{PORT}*/, 4);'],
            'megapiShowExternalLedTimeCodesSetup': ['rgbled_/*{PORT}*/.fillPixelsBak(0, 2, 1);'],
            'megapiShowExternalLedTimeCodesCode': (args) => {
...
rgbled_${args.PORT}.show();`
            ...
            'megapiShowExternalLedCodesLib': ['MeRGBLed rgbled_/*{PORT}*/(/*{PORT}*/, 4);'],
            'megapiShowExternalLedCodesSetup': ['rgbled_/*{PORT}*/.fillPixelsBak(0, 2, 1);'],
            'megapiShowExternalLedCodesCode': (args) => {
                const color = formatColorData(args.COLOR);
                return `
rgbled_${args.PORT}.setColor(${args.LED_POSTION},  ${color});
rgbled_${args.PORT}.show();`
            },
            'megapi_show_external_led': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
                    let rgb = color2RGB(args.COLOR);
                    await device.asyncWriteProtocol('ff55', devProtocol.show_external_led(index, Number(args.PORT), Number(args.LED_POSTION), rgb));
            ...
            'megapiShowExternalLedRgbCodesLib': ['MeRGBLed rgbled_/*{PORT}*/(/*{PORT}*/, 4);'],
            'megapiShowExternalLedRgbCodesSetup': ['rgbled_/*{PORT}*/.fillPixelsBak(0, 2, 1);'],
            'megapiShowExternalLedRgbCodesCode': (args) => {
                if (args.R == "None") {
                    args.R = 0;
...
rgbled_${args.PORT}.show();`
            ...
            'megapiShowAllLedstripColorCodesLib': ['MeRGBLed rgbled_/*{PORT}*/_/*{SLOT}*/(/*{PORT}*/, /*{SLOT}*/, 32);'],
            'megapiShowAllLedstripColorCodesSetup': ['rgbled_/*{PORT}*/_/*{SLOT}*/.fillPixelsBak(0, 2, 1);'],
            'megapiShowAllLedstripColorCodesCode': (args) => {
                const rgb = color2RGB(args.COLOR_LIST);
                return `
rgbled_${args.PORT}_${args.SLOT}.setColor(0, ${rgb});
rgbled_${args.PORT}_${args.SLOT}.show();`
            },
            'megapi_show_all_ledstrip_color': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
            ...
            'megapiShowLedstripColorCodesLib': ['MeRGBLed rgbled_/*{PORT}*/_/*{SLOT}*/(/*{PORT}*/, /*{SLOT}*/, 32);'],
            'megapiShowLedstripColorCodesSetup': ['rgbled_/*{PORT}*/_/*{SLOT}*/.fillPixelsBak(0, 2, 1);'],
            'megapiShowLedstripColorCodesCode': (args) => {
                if (args.POS == "None") {
                    args.POS = 0;
                } else if (Number(args.POS) < 0) {
                    args.POS = 0;
                } else if (Number(args.POS) > 255) {
                    args.POS = 255;
                }
...
rgbled_${args.PORT}_${args.SLOT}.show();`
            ...
            'megapiShowLedstripRbgCodesLib': ['MeRGBLed rgbled_/*{PORT}*/_/*{SLOT}*/(/*{PORT}*/, /*{SLOT}*/, 32);'],
            'megapiShowLedstripRbgCodesSetup': ['rgbled_/*{PORT}*/_/*{SLOT}*/.fillPixelsBak(0, 2, 1);'],
            'megapiShowLedstripRbgCodesCode': (args) => {
                if (args.POS == "None") {
                    args.POS = 0;
                } else if (Number(args.POS) < 0) {
                    args.POS = 0;
...
rgbled_${args.PORT}_${args.SLOT}.show();`
            ...
            'megapiShow7SegmentsNumberCodesLib': ['Me7SegmentDisplay seg7_/*{PORT}*/(/*{PORT}*/);'],
            'megapiShow7SegmentsNumberCodesCode': (args) => {
                if (args.NUMBER == "None") {
                    args.NUMBER = 0;
...
seg7_${args.PORT}.display(float(${args.NUMBER}));`
            ...
            'megapiDetectExternalLightCodesLib': ['MeLightSensor lightsensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_external_light': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectExternalLoudnessCodesLib': ['MeSoundSensor soundsensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_external_loudness': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetecTemperatureCodesLib': ['MeTemperature temperature_/*{PORT}*/_/*{SLOT}*/(/*{PORT}*/, /*{SLOT}*/);'],
            'megapi_detec_temperature': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectHumitureCodesLib': ['MeHumiture humiture_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_humiture': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiEventTouchCodesLib': ['MeTouchSensor touchSensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_event_touch': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectCompassCodesLib': ['MeCompass compass_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_compass': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectFlameCodesLib': ['MeFlameSensor flameSensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_flame': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectGasCodesLib': ['MeGasSensor gasSensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_gas': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiEventPirMotionCodesLib': ['MePIRMotionSensor pir_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_event_pir_motion': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiEventButtonPressCodesLib': ['Me4Button buttonSensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_event_button_press': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiEventLimitSwitchCodesLib': ['MeLimitSwitch sw_/*{PORT}*/_/*{SLOT}*/(/*{PORT}*/, /*{SLOT}*/);'],
            'megapi_event_limit_switch': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectPotentiometerCodesLib': ['MePotentiometer potentiometer_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_potentiometer': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectJoystickCodesLib': ['MeJoystick joystick_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_joystick': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectInfraredCodesLib': ['MeInfraredReceiver ir_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_infrared': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex();
                    await device.asyncWriteProtocol('ff55', [
            ...
            'megapiDetectPm25SensorCodesLib': ['MePm25Sensor myMePm25Sensor_/*{PORT}*/(/*{PORT}*/);'],
            'megapi_detect_pm25_sensor': {
                onRun: async (args, app, device, block) => {
                    let index = await getIndex()
                    await device.asyncWriteProtocol('ff55', [
