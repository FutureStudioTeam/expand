...
class ExtNovapiMbuildAngleSensor {
        ...
        this.funcs = {
            'BLOCK_1574413926331': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.angle_sensor import angle_sensor_class
angle_sensor_${args.index} = angle_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1559703114098': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = angle_sensor_${args.index}.get_angle()
online_debug_respond(value)`
            ...
            'BLOCK_1559703352577': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `angle_sensor_${args.index}.reset_angle()`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1559703305709': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = angle_sensor_${args.index}.get_angle_speed()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1559703446126': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = angle_sensor_${args.index}.is_rotating_${args.dir}()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
...
export default ExtNovapiMbuildAngleSensor;