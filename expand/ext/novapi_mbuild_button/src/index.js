        ...
        this.funcs = {
            'BLOCK_1574066208471': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.button import button_class
button_${args.index} = button_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1559700086192': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = button_${args.index}.is_pressed()
online_debug_respond(value)`
            ...
            'BLOCK_1559700149950': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = button_${args.index}.get_count()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1559700240893': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `button_${args.index}.reset_count()`
                    await OnlineProtocol.writeScript(device, scriptString);
                    return value;
                }
            }
...
export default ExtNovapiMbuildButton;