...
class ExtNovapiMbuildDcMotorDriver {
        ...
        this.funcs = {
            'BLOCK_1574131619495': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.dc_motor_driver import dc_motor_driver_class
dc_motor_driver_${args.index} = dc_motor_driver_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1550542693995': {
                onRun: async (args, app, device, block) => {
                    ...
                    scriptString = `dc_motor_driver_${args.index}.set_power(${args.power})`
                    await OnlineProtocol.writeScript(device, scriptString);

                    await delay(time * 1000)
                    scriptString = `dc_motor_driver_${args.index}.set_power(0)`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1552189686600': {
...
`
            ...
            'BLOCK_1550542713045': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `dc_motor_driver_${args.index}.change_power(${args.power})
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
...
`
            ...
            'BLOCK_1550543162879': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = dc_motor_driver_${args.index}.get_power()
online_debug_respond(value)`
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_dc_motor_driver",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildDcMotorDriver;