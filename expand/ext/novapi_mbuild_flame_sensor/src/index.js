...
class ExtNovapiMbuildFlameSensor {
        ...
        this.funcs = {
            'BLOCK_1574153647229': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.flame_sensor import flame_sensor_class
flame_sensor_${args.index} = flame_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1560259418555': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = flame_sensor_${args.index}.is_activated()
online_debug_respond(value)`
            ...
            'BLOCK_1560259595150': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = flame_sensor_${args.index}.get_value()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
...
export default ExtNovapiMbuildFlameSensor;