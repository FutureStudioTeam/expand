...
class ExtNovapiMbuildHumituresensor {
        ...
        this.funcs = {
            'BLOCK_1573813798844': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.humiture_sensor import humiture_sensor_class
humiture_sensor_${args.index} = humiture_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1573813807006': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = humiture_sensor_${args.index}.get_temperature("celsius")
online_debug_respond(value)`
            ...
            'BLOCK_1573813829362': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = humiture_sensor_${args.index}.get_temperature("fahrenheit")
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1573813831637': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = humiture_sensor_${args.index}.get_relative_humidity()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_humituresensor",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildHumituresensor;