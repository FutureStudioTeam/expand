...
class ExtNovapiMbuildIrTransceiver {

    constructor() {
        this.funcs = {
            'BLOCK_IR_1574244770906': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.ir_transceiver import ir_transceiver_class
ir_transceiver_${args.NUM} = ir_transceiver_class("${args.PORT}", "${args.INDEX}")`
            ...
            'BLOCK_1559548002545': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    let temp_string = string_encode(args.message)
                    scriptString = `ir_transceiver_${args.index}.send(${temp_string})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1559640370045': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = ir_transceiver_${args.index}.receive()
online_debug_respond(value)`
            ...
            'BLOCK_1559641582335': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `ir_transceiver_${args.index}.learn(${args.record_id})`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(3000)
                }
            },
            'BLOCK_1559641607063': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `ir_transceiver_${args.index}.send_learned_result(${args.record_id})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1559641693311': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = ir_transceiver_${args.index}.is_receive(${args.cmd})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
...
online_debug_respond(value)`
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_ir_transceiver",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildIrTransceiver;