        ...
        this.funcs = {
            'BLOCK_1574155174254': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.joystick import joystick_class
joystick_${args.index} = joystick_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1559707965944': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = joystick_${args.index}.is_activated('${args.dir}')
online_debug_respond(value)`
            ...
            'BLOCK_1559708436297': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = joystick_${args.index}.get_value("${args.axis}")
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
...
export default ExtNovapiMbuildJoystick;