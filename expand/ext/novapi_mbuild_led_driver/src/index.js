...
class ExtNovapiMbuildLedDriver {
        ...
        this.funcs = {
            'BLOCK_1574391311283': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.led_driver import led_driver_class
led_driver_${args.index} = led_driver_class("${args.PORT}", "${args.NUM}")`
            ...
            'block1550541402675CodesCode': (args) => {
                ...
                return `led_driver_${args.index}.set_block(${dataList.length}, [${dataList}])`;
            },
            'BLOCK_1550541402675': {
                onRun: async (args, app, device, block) => {
                    let color_array = formatLedStripFace(args.facePanel);
                    ...
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.set_block(${dataList.length}, [${dataList}])`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'block1550541816855CodesCode': (args) => {
                ...
                return `led_driver_${args.index}.set_single(${args.POSITION}, ${r}, ${g}, ${b})`;
            },
            'BLOCK_1550541816855': {
                onRun: async (args, app, device, block) => {
                    let color = args.color.replace('#', '0x');
                    let scriptString = []
                    let r = Number(color) >> 16;
                    let g = Number(color) >> 8 & 0xff;
                    let b = Number(color) & 0xff;
                    scriptString = `led_driver_${args.index}.set_single(${args.POSITION}, ${r}, ${g}, ${b})
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1550541885794': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.set_single(${args.position}, ${args.r}, ${args.g}, ${args.b})
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1550541898810': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.set_single(${args.position}, 0, 0, 0)
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1550541910495': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.set_all(0, 0, 0)
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1550541922579': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.set_${args.color}(${args.position}, ${args.value})
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1550541936947': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.change_${args.color}(${args.position}, ${args.value})
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1550541949702': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_driver_${args.index}.set_mode('${args.mode}')
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            }
...
export default ExtNovapiMbuildLedDriver;