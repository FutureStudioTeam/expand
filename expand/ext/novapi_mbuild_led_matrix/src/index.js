...
class ExtNovapiMbuildLedMatrix {
        ...
        this.funcs = {
            'LED_PANEL_INIT_1': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.led_matrix import led_matrix_class
led_matrix_${args.NUM} = led_matrix_class("${args.PORT}", "${args.INDEX}")`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'block1574043705050CodesCode': (args) => {
                return `led_matrix_${args.index}.show_image(${'"' + matrix2hexStr(args.panel) + '"'}, time_s = ${args.time})`;
            },
            'BLOCK_1574043705050': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    let panel = matrix2hexStr(args.panel)

                    let time = parseFloat(args.time);
                    if ((time <= 0) || (isNaN(time))) {
                        return;
                    }
                    scriptString = `led_matrix_${args.index}.show_image("${panel}")`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(time * 1000)
                    scriptString = `led_matrix_${args.index}.clear()`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'block1574043725470CodesCode': (args) => {
                return `led_matrix_${args.index}.show_image(${'"' + matrix2hexStr(args.panel) + '"'})`;
            },
            'BLOCK_1574043725470': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
            ...
            'block1574043736010CodesCode': (args) => {
                return `led_matrix_${args.index}.show_image(${'"' + matrix2hexStr(args.panel) + '"'}, ${args.x}, ${args.y})`;
            },
            'BLOCK_1574043736010': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    let panel = matrix2hexStr(args.panel)
                    scriptString = `led_matrix_${args.index}.show_image("${panel}", ${args.x}, ${args.y})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1574043738662': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    let temp_string = string_encode(args.string)
                    scriptString = `led_matrix_${args.index}.show(${temp_string}, wait=False)`
            ...
            'BLOCK_1574043741731': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    let temp_string = string_encode(args.string)
                    scriptString = `led_matrix_${args.index}.show(${temp_string}, wait=True)`
                    await OnlineProtocol.writeScript(device, scriptString);
                    ...
                    do {
                        await delay(100);
                        scriptString = `value = (led_matrix_${args.index}.get_show_status())
online_debug_respond(value)`
            ...
            'BLOCK_1574043745383': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    let temp_string = string_encode(args.string)
                    scriptString = `led_matrix_${args.index}.show(${temp_string}, ${args.pos_x}, ${args.pos_y})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1574043751221': {
            ...
            'BLOCK_1574043760063': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_matrix_${args.index}.set_pixel(${args.pos_x}, ${args.pos_y}, True)`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1574043763532': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_matrix_${args.index}.set_pixel(${args.pos_x}, ${args.pos_y}, False)`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1574043766677': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `led_matrix_${args.index}.toggle_pixel(${args.pos_x}, ${args.pos_y})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1574043769245': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (led_matrix_${args.index}.get_pixel(${args.pos_x}, ${args.pos_y}))
online_debug_respond(value)`
...
export default ExtNovapiMbuildLedMatrix;