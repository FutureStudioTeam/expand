...
class ExtNovapiMbuildMagneticSensor {
        ...
        this.funcs = {
            'BLOCK_1574652459444': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.magnetic_sensor import magnetic_sensor_class
magnetic_sensor_${args.index} = magnetic_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1565579151187': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = magnetic_sensor_${args.index}.is_activated()
online_debug_respond(value)`
            ...
            'BLOCK_1565579556728': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = magnetic_sensor_${args.index}.get_count()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1565579668227': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `magnetic_sensor_${args.index}.reset_count()`
                    await OnlineProtocol.writeScript(device, scriptString);
                    return value;
                }
            }
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_magnetic_sensor",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildMagneticSensor;