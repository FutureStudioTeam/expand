...
class ExtNovapiMbuildMotionSensor {

    constructor() {
        this.funcs = {
            'BLOCK_MOTION_SENSOR_1574244770906': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.motion_sensor import motion_sensor_class
motion_sensor_${args.NUM} = motion_sensor_class("${args.PORT}", "${args.INDEX}")`
            ...
            'BLOCK_1560309782310': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (motion_sensor_${args.index}.is_${args.dir}())
online_debug_respond(value)`
            ...
            'BLOCK_1560309306419': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (motion_sensor_${args.index}.is_shaked('${args.strength}'))
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1560309209133': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (motion_sensor_${args.index}.get_shake_strength())
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1560314176719': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (motion_sensor_${args.index}.get_${args.posture}())
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1560314360278': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (motion_sensor_${args.index}.get_rotation('${args.axis}'))
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_1560320781489': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `motion_sensor_${args.index}.reset_rotation('${args.axis}')`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            }
        };
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_motion_sensor",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildMotionSensor;