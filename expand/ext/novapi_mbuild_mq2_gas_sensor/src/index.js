...
class ExtNovapiMbuildMq2GasSensor {
        ...
        this.funcs = {
            'BLOCK_1574232106708': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.mq2_gas_sensor import mq2_gas_sensor_class
mq2_gas_sensor_${args.index} = mq2_gas_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1560258942584': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = mq2_gas_sensor_${args.index}.is_activated('${args.sen}')
online_debug_respond(value)`
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_mq2_gas_sensor",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildMq2GasSensor;