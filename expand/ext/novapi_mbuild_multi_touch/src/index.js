...
class ExtNovapiMbuildMultiTouch {
        ...
        this.funcs = {
            'BLOCK_1574244770906': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.multi_touch import multi_touch_class
multi_touch_${args.index} = multi_touch_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1559810784185': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = multi_touch_${args.index}.is_activated(${args.tp_num})
online_debug_respond(value)`
            ...
            'BLOCK_1559811080887': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `multi_touch_${args.index}.set_sensitivity(${args.sensitivity})
`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            }
...
export default ExtNovapiMbuildMultiTouch;