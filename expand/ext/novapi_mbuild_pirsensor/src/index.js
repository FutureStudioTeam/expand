...
class ExtNovapiMbuildPirsensor {
        ...
        this.funcs = {
            'BLOCK_1574218254527': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.pir_sensor import pir_sensor_class
pir_sensor_${args.index} = pir_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'event_pir': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = pir_sensor_${args.index}.is_activated()
online_debug_respond(value)`
            ...
            'pir_count': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = pir_sensor_${args.index}.get_count()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'pir_count_make_zero': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `pir_sensor_${args.index}.reset_count()`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            }
        };
...
export default ExtNovapiMbuildPirsensor;