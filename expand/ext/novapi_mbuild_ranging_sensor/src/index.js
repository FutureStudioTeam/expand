...
class ExtNovapiMbuildRangingSensor {
...
ranging_sensor_${args.index} = ranging_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'op_ranging': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = ranging_sensor_${args.index}.get_distance()
...
online_debug_respond(value)`
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_ranging_sensor",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildRangingSensor;