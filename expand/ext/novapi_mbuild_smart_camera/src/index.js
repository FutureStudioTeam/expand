...
class ExtNovapiMbuildSmartCamera {
...
smart_camera_${args.NUM} = smart_camera_class("${args.PORT}", "${args.INDEX}")`
            ...
            'SMART_CAMERA_CHANGE_CCC': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `smart_camera_${args.NUM}.set_mode("color")`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(200);
                }
            },
            'SMART_CAMERA_START_LEARN_CCC': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `smart_camera_${args.NUM}.learn(${args.SIG}, "until_button")`
                    await OnlineProtocol.writeScript(device, scriptString);
                    ...
                    do {
                        await delay(1000);
                        scriptString = `value = smart_camera_${args.NUM}.detect_sign(${args.SIG})
online_debug_respond(value)`
                        state = await OnlineProtocol.readScript(device, scriptString);
                        count--;
                    } while ((state == 0) && (count > 0));
                }
            },
...
online_debug_respond(value)`
            ...
            'SMART_CAMERA_LOCATION': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.detect_sign_location(${args.SIG}, "${args.LOCATION}")
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_VALUE': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_sign_${args.TYPE}(${args.SIG})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_SET_LED': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.${args.TYPE}()
online_debug_respond(value)`
            ...
            'SMART_CAMERA_RESET': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `smart_camera_${args.NUM}.reset()`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(9000);
                }
            },
...
smart_camera_${args.NUM} = smart_camera_class("${args.PORT}", "${args.INDEX}")`
            ...
            'SMART_CAMERA_CHANGE_LINE': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `smart_camera_${args.NUM}.set_mode("line")`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(200);
                }
            },
            'BLOCK_25650950268712': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.detect_label(${args.LABEL})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_25650950268713': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_label_${args.COORD}(${args.LABEL})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_GET_LINE_VALUE': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_vector_${args.TYPE}()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_DETECT_CROSS': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.detect_cross()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_GET_CROSS_VALUE': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_cross_${args.TYPE}()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_25650950268718': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_cross_road()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'BLOCK_25650950268719': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_cross_angle(${args.sn})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_SET_LINE_MODE': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `smart_camera_${args.NUM}.set_line("${args.LINEMODE}")`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'SMART_CAMERA_INIT_3': {
...
smart_camera_${args.NUM} = smart_camera_class("${args.PORT}", "${args.INDEX}")`
            ...
            'SMART_CAMERA_SET_KP': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `smart_camera_${args.NUM}.set_kp(${args.KP})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'SMART_CAMERA_DIFF_SPEED_CCC': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_sign_diff_speed(${args.SIG},"${args.AXIS}",${args.TARGET})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_DIIF_SPEED_LABEL': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_label_diff_speed(${args.LABEL},"${args.AXIS}",${args.TARGET})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_DIFF_SPEED_LINE': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.get_follow_vector_diff_speed()
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_IS_LOCK_CCC': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.is_lock_sign(${args.SIG},"${args.AXIS}",${args.TARGET})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
            ...
            'SMART_CAMERA_IS_LOCK_LABEL': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = smart_camera_${args.NUM}.is_lock_label(${args.LABEL},"${args.AXIS}",${args.TARGET})
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
...
export default ExtNovapiMbuildSmartCamera;