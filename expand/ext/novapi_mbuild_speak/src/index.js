...
class ExtNovapiMbuildSpeak {

    constructor() {
        this.funcs = {
            'BLOCK_SPEAKER_1574244770906': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.speaker import speaker_class
speaker_${args.NUM} = speaker_class("${args.PORT}", "${args.INDEX}")`
            ...
            'BLOCK_1553621437508': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `speaker_${args.index}.play_tone(${args.note}, ${args.beat})`
                    await OnlineProtocol.writeScript(device, scriptString);

                    await delay(args.beat * 1000 + 100);
                }
            },
            'BLOCK_1552140203501': {
                onRun: async (args, app, device, block) => {
            ...
            'BLOCK_1551148572564': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `speaker_${args.index}.play_tone(${args.hz}, ${args.time})`
                    await OnlineProtocol.writeScript(device, scriptString);

                    await delay(args.time * 1000 + 200);
                }
            },
            'BLOCK_1553668530911': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `speaker_${args.index}.play_tone(${args.hz})`
                    await OnlineProtocol.writeScript(device, scriptString);
                }
            },
            'BLOCK_1552140311744': {
                onRun: async (args, app, device, block) => {
                    ...
                    scriptString = `speaker_${args.index}.play_melody(${melody})`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(100);
                }
            },
            'BLOCK_1552140277681': {
                onRun: async (args, app, device, block) => {
                    ...
                    do {
                        await delay(100);
                        scriptString = `value = (speaker_${args.index}.is_playing())
online_debug_respond(value)`
            ...
            'BLOCK_1552140248164': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `speaker_${args.index}.stop_sounds()`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(100);
                }
            },
            'BLOCK_1552140340843': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `speaker_${args.index}.change_volume(${args.volume})`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(100);
                }
            },
            'BLOCK_1552140362788': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `speaker_${args.index}.set_volume(${args.volume})`
                    await OnlineProtocol.writeScript(device, scriptString);
                    await delay(100);
                }
            },
            'BLOCK_1552140447611': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = (speaker_${args.index}.get_volume())
online_debug_respond(value)`
...
export default ExtNovapiMbuildSpeak;