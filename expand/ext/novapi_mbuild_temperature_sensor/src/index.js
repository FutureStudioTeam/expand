...
class ExtNovapiMbuildTemperatureSensor {
        ...
        this.funcs = {
            'BLOCK_1574417422755': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `from mbuild.temp_sensor import temp_sensor_class
temp_sensor_${args.index} = temp_sensor_class("${args.PORT}", "${args.NUM}")`
            ...
            'BLOCK_1560257762145': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = temp_sensor_${args.index}.get_temperature("celsius")
online_debug_respond(value)`
            ...
            'BLOCK_1560258599092': {
                onRun: async (args, app, device, block) => {
                    let scriptString = [];
                    scriptString = `value = temp_sensor_${args.index}.get_temperature("fahrenheit")
online_debug_respond(value)`
                    let value = await OnlineProtocol.readScript(device, scriptString);
                    return value;
                }
    ...
    getInfo() {
        return {
            "id": "novapi_mbuild_temperature_sensor",
            "targets": [
                "novapi"
            ],
            "codeTypes": [
...
export default ExtNovapiMbuildTemperatureSensor;