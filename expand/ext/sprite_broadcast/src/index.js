...
let threadList = []; // 多个broadcast同时发送，有一定概率会导致整个收发系统紊乱，所以这里采用线程串行的形式
// 扩展加载时调用，订阅硬件扩展发送的事件
function subscribeMessage(app) {
    appContext = app;
    if (!appCancelSubscribe) {
        appCancelSubscribe = appContext.subscribeMessage("robot2spriteBroadcast", ({
            type,
            msg,
            value
        }) => {
            if (type && type == 'cmd' && msg == 'threadFinish') {
                if (threadList[0].threadCallback) {
                    threadList[0].threadCallback();
                }
            } else {
...
// 向硬件扩展发送事件
function broadcast(msg, value) {
    msg = String(msg).replace(/^\s+|\s+$/g, '');
    if (msg == '') return;
    console.log('=>', msg);
    threadList.push({
        msg,
...
function nextThread() {
    if (threadList.length == 1) {
        return new Promise(resolve => {
            createThread(threadList[0], resolve);
        })
    } else {
        // 当前线程不是第一条，那么创建一个promise，并将要发送的动作封装成函数，当上一条线程结束后调用。形成链表串联起来
        return new Promise(resolve => {
            let thread = threadList[threadList.length - 1];
            threadList[threadList.length - 2].nextPromise = function() {
                createThread(thread, resolve);
            }
        })
    }
}

function createThread(thread, resolve) {
    appContext.sendMessage("sprite2robotBroadcast", thread);
    thread.threadCallback = function() {
        clearTimeout(thread.timeoutT);
        thread.threadCallback = null;
        resolve();
        handleNextThread();
    };
    thread.timeoutT = setTimeout(function() {
        thread.threadCallback = null;
        resolve();
        handleNextThread();
    }, 5000);
...
function handleNextThread() {
    ...
    thread.nextPromise && thread.nextPromise();
}
// 帽子块添加时调用
function addBroadcastListener(callback) {
    broadcastCallback = callback;
}
// 帽子块移除时调用
function removeBroadcastListener() {
...
class ExtSpriteBroadcast {

    constructor() {
        this.funcs = {
            'BLOCK_1546412605792': {
                onRun: async (args, app, device, block) => {
                    await broadcast(args.MESSAGE);
                }
            },
            'BLOCK_1546413597853': {
                onRun: async (args, app, device, block) => {
                    await broadcast(args.MESSAGE, args.VALUE);
                }
            },
            'BLOCK_1546413700881': {
                onRun: (args, app, device, block) => {
                    if (currBroadcast == args.MESSAGE) {
                        return true;
                    }
                    return false;
                },
                onAdd: (app, device, block) => {
                    addBroadcastListener(function(msg) {
                        currBroadcast = msg;
            ...
            'BLOCK_1546415356012': {
                onRun: (args, app, device, block) => {
                    return broadcastMap[args.MESSAGE];
                },
...
export default ExtSpriteBroadcast;